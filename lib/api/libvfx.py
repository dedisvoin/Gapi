...
if __name__ == '__main__':
    ...
    p.set('color',(250,250,100))
    p.SPEED_DURATION = 180
    p.SPEED = Vector2(0,6)  
    p.RADIUS = 5

    p.RESIZE_START_TIME = 200
    p.COLLIDER_BOUNSING = Vector2(0.2,0.2)
    p.COLLIDER_TRENIE = Vector2(0.2,0.2)
    p.NAME = 's'
    ...
    p2.set('color',(250,80,10))
    p2.SPEED_DURATION = 10
    p2.SPEED = Vector2(0,-1)  
    p2.RADIUS_RESIZE = -0.1
    p2.LIGHTNING = True
    p2.LIGHT_COLOR_FROM_PARTICLE_COLOR = True
    p2.LIGHT_STRANGE_BY_SIZE = True
    p2.LIGHT_SIZE = 5
    p2.LIGHT_STRANGE = 0.89
    p2.SHAPE_RENDERER = False
    p2.SPEED_ANGLE = -90
    p2.LIGHT_SHAPE = particle_shapes.BLEND_CIRCLE
    ...
    def update(particle):
        ...
        if particle.NAME == 's':
            p2.RADIUS = particle.RADIUS+8
            p2.COLOR = particle.COLOR
            space.add(p2, c, 1, 2)
